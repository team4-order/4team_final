...
export default {
  ...
  methods: {
    fetchOrderList() {
      ...
      axios.get(`http://ec2-13-209-231-193.ap-northeast-2.compute.amazonaws.com:8080/api/deliveries/status/${this.mutableBusinessId}`)
        .then(response => {
    ...
    handleRowClick(row) {
      ...
      window.location.href = `http://easyoms.store/admin/deliverydetail/${orderNumber}`;
    },
  },
  computed: {
    totalPages() {
