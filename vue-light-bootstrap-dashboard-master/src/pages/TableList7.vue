      ...
      <div class="row">
        <div class="col-12">
          <card class="strpied-tabled-with-hover" body-classes="table-full-width table-responsive">
            ...
            <!-- <base-dropdown :initial-title="dropdownTitle" @item-selected="updateTitle" ref="dropdown">
...
export default {
  ...
  watch: {
    ...
    searchQuery(newVal) {
    if (newVal !== '' && this.selectedStorageCode !== '') {
      this.fetchGoodsList(this.selectedStorageCode);
    }
  }
  },
  mounted() {
  ...
  updateTitle(selectedItem) {
    this.dropdownTitle = selectedItem; // 선택된 항목으로 타이틀 업데이트
  },
    ...
    axios.get(`http://localhost:8080/api/total/storage/${storageCode}`)
      ...
      this.goodsList.data = filteredData.map(goods => ({
        ...goods,
        visible: false,
